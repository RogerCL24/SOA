...
void copy_user_mem(int pid, char* start, char* dest, size) {
        ...
        // b)
        int page = (int)start >> 12;

        // c)
        struct task_struct *target = find_task_by_pid(pid);

        page_table_entry *target_pt = get_PT(target);
        int frame = get_frame(target_pt, page);
        ...
        int page_offset = (int)start & 0xFFF;
        copy_data(mapped_addr + page_offset, dest, size);

        // e)
        del_ss_page(my_pt, LOGICAL_TEMP);
        set_cr3(get_DIR(current()));
