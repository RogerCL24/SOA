...
{	
	...
	child->task.exit_status = 0;
	child->task.w_blocked = 0;
	INIT_LIST_HEAD(&(child->task.zombies));
	INIT_LIST_HEAD(&(child->task.ready));
	INIT_LIST_HEAD(&(child->task.blocked));
	INIT_LIST_HEAD(&(child->task.sibling));

	child->task.parent = current();

	list_add_tail(&(child->task.sibling), &(current()->ready));
	/*
...
{  
	...
	ts->exit_status = error;
	// Si NO es idle, nos eliminamos de la lista de hijos del padre
	struct task_struct *parent = ts->parent;
	if (parent != NULL) { 
		list_del(&ts->sibling);
		list_add_tail(&ts->sibling, &parent->zombies);
	}

	struct list_head *pos, *tmp;
	list_for_each_safe(pos, tmp, &ts->ready) {
		struct task_struct *child = list_entry(pos, struct task_struct, sibling);
		list_del(pos);
		list_add_tail(&child->sibling, &idle_task->ready);
		child->parent = idle_task;
	}

	list_for_each_safe(pos, tmp, &ts->zombies) {
		struct task_struct *child = list_entry(pos, struct task_struct, sibling);
		list_del(pos);
		list_add_tail(&child->sibling, &idle_task->zombies);
		child->parent = idle_task;
	}

	list_for_each_safe(pos, tmp, &ts->blocked) {
		struct task_struct *child = list_entry(pos, struct task_struct, sibling);
		list_del(pos);
		list_add_tail(&child->sibling, &idle_task->blocked);
		child->parent = idle_task;
	}
	...
	update_process_state_rr(ts, &zombie);
	if (ts->w_blocked) unblock(parent);
	sched_next_rr();
}
...
int sys_waitpid(int pid, int *status) {
	...
	list_for_each(pos, &ts->ready) {
		struct task_struct *child = list_entry(pos, struct task_struct, sibling);		 if (child->PID == pid) {
			child->w_blocked = 1; 
			block();
