...
{	
	...
	list_add_tail(&(child->task.sibling), &(current()->child_list));
...
{  
	...
	list_for_each_safe(pos, tmp, &ts->child_list) {
		struct task_struct *child = list_entry(pos, struct task_struct, sibling);
		list_del(pos);
		list_add_tail(&child->sibling, &idle_task->child_list);
...
/*
 * Si tics == 0, cancelar el timeout programado y return de los ticks restantes
 * Si tics > 0, programar la ejecucion de f dentro de `tics` ticks y
 * devolver los ticks restantes de la programacion anterior (si habia)
 */
int sys_timeout(int tics, void (*f)(void)) {

	if (tics < 0) return -EINVAL;
	if (tics > 0 && f == NULL) return -EINVAL;
